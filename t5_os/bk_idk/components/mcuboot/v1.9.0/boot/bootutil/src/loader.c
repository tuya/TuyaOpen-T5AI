                         ...
                         struct boot_status *bs)
{
    ...
    if ((bk_boot_read_ota_confirm(OVERWRITE_CONFIRM))) {
        BOOT_LOG_INF("confirm set, do upgrade");
        swap_type = BOOT_SWAP_TYPE_TEST;
    } else {
        BOOT_LOG_INF("confirm unset, boot");
    ...
    if (BOOT_IS_UPGRADE(swap_type)) {
        ...
        BOOT_LOG_INF("validate OTA slot result %d",fih_rc);
        if (fih_not_eq(fih_rc, FIH_SUCCESS)) {
#if CONFIG_OTA_CONFIRM_UPDATE
...
#else /* MCUBOOT_HW_ROLLBACK_PROT && CONFIG_ANTI_ROLLBACK */
    (void) (state);
...
context_boot_go(struct boot_loader_state *state, struct boot_rsp *rsp)
{
    ...
    IMAGES_ITER(BOOT_CURR_IMG(state)) {
        ...
        if (BOOT_SWAP_TYPE(state) != BOOT_SWAP_TYPE_NONE) {
            ...
            flash_area_erase_fast(get_flash_map_offset(1), 4096);
            flash_area_erase_fast(get_flash_map_offset(1) + get_flash_map_size(1) - 4096, 4096);
#endif
#else
...
boot_select_or_erase(struct boot_loader_state *state)
{
    ...
    if (active_swap_state->magic != BOOT_MAGIC_GOOD ||
        ...
        rc = flash_area_erase(fap, 0, flash_area_get_erase_size(fap));
        assert(rc == 0);
...
boot_select_or_erase(struct boot_loader_state *state)
{
    ...
    if( magic == XIP_SET && copy_done == XIP_SET ) {
        ...
        BOOT_LOG_ERR("xip ota image download aborted,erase image\r\n");
