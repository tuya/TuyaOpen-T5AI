...
class Genbl1:
    ...
    def sig_sch_to_scheme(self):
        ...
        elif (self.sig_sch == 'rsa3072'):
            return 'RSASSA_3072_PKCS_V21_SHA256'
        else:
    ...
    def check_key(self):
        ...
        if self.action_type == 'sign':
            if (self.prvkey == '') or (self.prvkey == None) or (os.path.exists(self.prvkey) == False):
                logging.error(f'missing private key')
                exit(1)

        if (self.pubkey == '') or (self.pubkey == None) or (os.path.exists(self.pubkey) == False):
            logging.error(f'missing public key')
            exit(1)

    def __init__(self, action_type='sign', sec_boot=True, sig_sch='ec256', prvkey='', pubkey='bl1_ec256_pubkey.pem',outfile =''):
